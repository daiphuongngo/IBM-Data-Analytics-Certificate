...
# -*- coding: utf-8 -*-
"""
Created on Fri Apr 10 19:37:33 2020
...
Graded Question 1
1/1 point (ungraded)
How would you add one to each element in the column df["a"] and assign it back 
to the column df["a"]?"""
df["a"]=df["a"]+1
...
"""Graded Question 2
What is the correct line of code to perform One-hot encoding on the 
column 'fuel'?"""
...
Data Wrangling is the process of converting data from the initial format to a 
format that may be better for analysis.
...
"""As we can see, several question marks appeared in the dataframe; those are 
missing values which may hinder our further analysis.
...
data cannot be used for prediction; therefore any row now without price data 
is not useful to us """
...
Correct data format
We are almost there!
The last step in data cleaning is checking and making sure that all data is in 
...
Numerical variables should have type 'float' or 'int', and variables with 
strings such as categories should have type 'object'. For example, 'bore' and 
'stroke' variables are numerical values that describe the engines, so we should 
expect them to be of the type 'float' or 'int'; however, they are shown as 
...
"""Wonderful!
Now, we finally obtain the cleaned dataset with no missing values and all data 
...
allows the researcher to make the meaningful comparison.

Example
Transform mpg to L/100km:
In our dataset, the fuel consumption columns "city-mpg" and "highway-mpg" are 
represented by mpg (miles per gallon) unit. Assume we are developing an 
...
a similar range. Typical normalizations include scaling the variable so the 
variable average is 0, scaling the variable so the variance is 1, or scaling 
...
df['horesepower']=df['horsepower'].astype(int, copy=True) #copy and make a new column with the same name but type int
...
"""=======================
Bins visualization
Normally, a histogram is used to visualize the distribution of bins we created 
above."""
%matplotlib inline
import matplotlib as plt
from matplotlib import pyplot
...
An indicator variable (or dummy variable) is a numerical variable used to label 
categories. They are called 'dummies' because the numbers themselves don't have 
inherent meaning.
...
"""We now have the value 0 to represent "gas" and 1 to represent "diesel" in 
the column "fuel-type". We will now insert this column back into our original 
dataset."""
...
"""====================
Question #5: 
Merge the new dataframe to the original dataframe then drop the 
column 'aspiration' """
